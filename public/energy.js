...
(function () {
  var safeHTML    = EFC.safeHTML;
  var escapeHTML  = EFC.escapeHTML;
  ...
  function getFilteredDisruptions() {
    const { airline, region, country, impactType, severity, search } = EnergyState.filters;
    const q = search.toLowerCase().trim();
    return EnergyState.disruptions.filter(d => {
  ...
  function sortDisruptions(rows) {
    const { col, dir } = EnergyState.sort;
  ...
  function computeKPIs(disruptions, fuelPrices, airports) {
    ...
    const riskThreshold = EnergyState.summerMode ? 10 : 7;
  ...
  function renderKPIs(disruptions, fuelPrices, airports) {
    ...
    const cards = [
      { label: 'Avg. Cover Days', value: kpi.avgCoverDays != null ? kpi.avgCoverDays.toFixed(1) : '—', extra: EnergyState.summerMode ? '<span class="kpi-delta neutral">summer-adjusted</span>' : '', sub: 'Modelled estimates · ' + kpi.totalAirports + ' airports', accent: '--primary' },
      { label: 'Airports at Risk', value: kpi.airportsAtRisk + ' / ' + kpi.totalAirports, extra: '', sub: EnergyState.summerMode ? 'Below 10 days cover (summer)' : 'Below 7 days cover', accent: kpi.airportsAtRisk > 0 ? '--error' : '--c-low', clickable: true, target: '#airports' },
  ...
  function renderTable(rows, tbodyId, countId) {
    ...
    const html = rows.map(d => {
      ...
      const chips = vis.map(r => `<span class="route-chip">${escapeHTML(r)}</span>`).join('')
                  + (more > 0 ? `<span class="route-chip-more">+${more}</span>` : '');
      const ncls  = d.cancellations >= 40 ? 'high-num' : d.cancellations >= 15 ? 'med-num' : '';
      return `<tr tabindex="0" role="button" aria-label="View details: ${escapeHTML(d.airline)} event ${escapeHTML(d.id)}" data-id="${escapeHTML(d.id)}">
        <td class="td-primary"><div class="airline-cell">${d.airline_code ? `<span class="airline-code">${escapeHTML(d.airline_code)}</span>` : ''}${escapeHTML(d.airline || d.summary || 'News item')}</div></td>
        <td class="${isOverview ? 'col-region' : ''}">${escapeHTML(d.region)}</td>
  ...
  function renderFuelChart() {
    const prices = EnergyState.fuelPrices;
  ...
  function renderAirportCards() {
    ...
    const html = EnergyState.airports.map(a => {
      ...
      return `
      <div class="airport-card" data-airport="${escapeHTML(a.code)}">
        <div class="airport-card-header">
          <span class="airport-code-badge">${escapeHTML(a.code)}</span>
          <div>
            <div class="airport-name">${escapeHTML(a.name)}</div>
            <div class="airport-city">${escapeHTML(a.city)}, ${escapeHTML(a.country)}</div>
          </div>
        </div>
        ...
        <div class="airport-stats">
          ...
          <div class="airport-stat">
            ...
            <span class="badge ${depCls}" style="font-size:9px"><span class="badge-dot" aria-hidden="true"></span>${escapeHTML(a.import_dependency)}</span>
          </div>
        </div>
        <div class="airport-notes">${escapeHTML(a.notes)}</div>
      </div>`;
    }).join('');
  ...
  function updateMapMarkers() {
    ...
    const maxBurn = Math.max(...EnergyState.airports.map(a => a.daily_burn_ml));

    EnergyState.airports.forEach(a => {
      const days = getAirportCoverDays(a);
      const cls = getCoverDaysClass(days);
      ...
      marker.bindTooltip(
        `<strong>${escapeHTML(a.code)} — ${escapeHTML(a.name)}</strong><br>${days.toFixed(1)} days cover<br><span style="color:${colorMap[cls]}">${getCoverDaysRiskLabel(days)}</span>`,
    ...
    const metaHTML = `
      <div>
        ...
        <div class="drawer-meta-grid">
          ...
          <div class="drawer-meta-item"><span class="drawer-meta-key">Import Dep.</span><span class="drawer-meta-val"><span class="badge ${depCls}" style="font-size:11px"><span class="badge-dot" aria-hidden="true"></span>${escapeHTML(airport.import_dependency)}</span></span></div>
      ...
      <div>
        ...
        <div class="drawer-ops-notes">${escapeHTML(airport.notes)}</div>
      </div>
      ${EnergyState.summerMode ? `<div>
  ...
  function renderRegionalChart() {
    ...
    const byRegion = {};
    for (const d of EnergyState.disruptions) byRegion[d.region] = (byRegion[d.region] || 0) + d.cancellations;
  ...
  function openDrawer(eventId) {
    const ev = EnergyState.disruptions.find(d => d.id === eventId);
    if (!ev) return;
    EnergyState.selectedEventId = eventId;
  ...
  function closeDrawer() {
    EnergyState.selectedEventId = null;
    ...
    EnergyState._lastFocusedRow?.focus();
    EnergyState._lastFocusedRow = null;
  }
  ...
  function renderDrawerContent(ev) {
    const metaHTML = `
      <div>
        ...
        <div class="drawer-meta-grid">
          <div class="drawer-meta-item"><span class="drawer-meta-key">Event ID</span><span class="drawer-meta-val">${escapeHTML(ev.id)}</span></div>
          <div class="drawer-meta-item"><span class="drawer-meta-key">Region</span><span class="drawer-meta-val">${escapeHTML(ev.region)}</span></div>
          ...
          <div class="drawer-meta-item"><span class="drawer-meta-key">Severity</span><span class="drawer-meta-val"><span class="badge ${escapeHTML(ev.severity)}" style="font-size:11px"><span class="badge-dot" aria-hidden="true"></span>${escapeHTML(ev.severity)}</span></span></div>
          <div class="drawer-meta-item"><span class="drawer-meta-key">Impact</span><span class="drawer-meta-val"><span class="impact-badge impact-${escapeHTML(ev.impact_type)}" style="font-size:11px">${impactLabel(ev.impact_type)}</span></span></div>
      ...
      <div>
        ...
        <p class="drawer-summary">${escapeHTML(ev.summary)}</p>
      </div>
      <div>
        <div class="drawer-section-label">Affected Routes</div>
        <div class="drawer-routes">${ev.routes.map(r => `<span class="drawer-route-chip">${escapeHTML(r)}</span>`).join('')}</div>
        <div class="drawer-airports">${ev.airports.map(a => `<span class="drawer-airport">${escapeHTML(a)}</span>`).join('')}</div>
      </div>
      ${ev.operational_notes ? `<div>
        <div class="drawer-section-label">Operational Notes</div>
        <div class="drawer-ops-notes">${escapeHTML(ev.operational_notes)}</div>
      </div>` : ''}
      ${ev.timeline && ev.timeline.length ? `<div>
        ...
        <div class="timeline" role="list">
          ${ev.timeline.map(t => `<div class="timeline-item" role="listitem">
              ...
              <div><div class="timeline-date">${escapeHTML(t.date)}</div><div class="timeline-note">${escapeHTML(t.note)}</div></div>
            </div>`).join('')}
        </div>
      </div>` : ''}
      <div>
        <div class="drawer-section-label">Source</div>
        <a href="${escapeHTML(ev.source_url)}" target="_blank" rel="noopener noreferrer" class="drawer-source-link" aria-label="${escapeHTML(ev.source_name)} (opens new tab)">
          <span aria-hidden="true">↗</span>${escapeHTML(ev.source_name)}
        </a>
      </div>`;
  ...
  function populateFilterOptions() {
    const airlines  = [...new Set(EnergyState.disruptions.map(d => d.airline).filter(Boolean))].sort();
    const regions   = [...new Set(EnergyState.disruptions.map(d => d.region).filter(Boolean))].sort();
    const countries = [...new Set(EnergyState.disruptions.map(d => d.country).filter(Boolean))].sort();
    const fill = (id, vals) => {
      const el = document.getElementById(id);
      if (!el) return;
      const first = el.options[0].outerHTML;
      safeHTML(el, first + vals.map(v => `<option value="${escapeHTML(v)}">${escapeHTML(v)}</option>`).join(''));
    };
  ...
  function applyAndRender() {
    ...
    renderKPIs(EnergyState.disruptions, EnergyState.fuelPrices, EnergyState.airports);
    renderTable(rows);
  ...
  function initFilters() {
    ...
    document.getElementById('filter-clear').addEventListener('click', () => {
      EnergyState.filters = { airline: '', region: '', country: '', impactType: '', severity: '', search: '' };
  ...
  function initTableSort() {
    document.querySelectorAll('thead th.sortable').forEach(th => {
      ...
      const activate = () => {
        ...
        EnergyState.sort.col === col
          ? (EnergyState.sort.dir = EnergyState.sort.dir === 'asc' ? 'desc' : 'asc')
          : (EnergyState.sort.col = col, EnergyState.sort.dir = col === 'updated_at' ? 'desc' : 'asc');
        applyAndRender();
      };
  ...
  function updateSortHeaders() {
    document.querySelectorAll('#disruptions-table thead th.sortable').forEach(th => {
      const active = th.dataset.col === EnergyState.sort.col;
      const icon = th.querySelector('.sort-icon');
      th.classList.remove('sort-asc', 'sort-desc');
      th.removeAttribute('aria-sort');
      if (active) {
        th.classList.add(EnergyState.sort.dir === 'asc' ? 'sort-asc' : 'sort-desc');
        th.setAttribute('aria-sort', EnergyState.sort.dir === 'asc' ? 'ascending' : 'descending');
        if (icon) icon.textContent = EnergyState.sort.dir === 'asc' ? '↑' : '↓';
      } else {
  ...
  function initSummerToggle() {
    ...
    toggle.addEventListener('click', () => {
      ...
      EnergyState.bannerDismissed = false;
    ...
    closeBtn.addEventListener('click', () => {
      EnergyState.bannerDismissed = true;
      updateSummerBanner();
    });
  }
  ...
  function updateSummerBanner() {
    ...
    banner.classList.toggle('hidden', !EnergyState.summerMode || EnergyState.bannerDismissed);
  }
  ...
  function onThemeChange(theme) {
    if (mapInitialized && mapInstance) {
      ...
      if (currentViewId() === 'map') renderMapView();
    }
    renderFuelChart();
    applyAndRender();
    if (currentViewId() === 'analytics') renderAnalyticsCharts();
  }

  function onSidebarChange(collapsed) {
  ...
  function filtersHTML() {
    return `
      ...
      <select class="filter-select" id="filter-airline" aria-label="Filter by airline">
        <option value="">All Airlines</option>
      </select>
      <select class="filter-select" id="filter-region" aria-label="Filter by region">
      ...
      <input
        ...
        aria-label="Search by route or airport"
        autocomplete="off"
      />
      <button class="filter-clear" id="filter-clear" type="button" aria-label="Clear all filters">Clear</button>
      <div class="filter-divider" aria-hidden="true"></div>
      <button class="summer-toggle" id="summer-toggle" type="button" aria-pressed="false" aria-label="Toggle summer peak demand mode">
